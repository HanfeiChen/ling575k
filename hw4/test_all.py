...
class TestOps:
    ...
    def test_exp(self):
        # test forward
        exps = ops.exp(Tensor(TestOps.batch))
        ...
        mean.backward()
        torch_exps = torch.tensor(TestOps.batch, requires_grad=True)
        torch_exps.mean().backward()
        np.testing.assert_allclose(exps.grad, torch_exps.grad.numpy())
