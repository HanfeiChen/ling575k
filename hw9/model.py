...
class PretrainedClassifier(nn.Module):
    ...
    def __init__(self, encoder: BertModel, num_labels: int):
        ...
        # DONE (~1 line): create a linear output layer that will consume
        # a representation from self.encoder, and produce logits over the labels
        # Note: store this linear layer as self.output
        self.output = nn.Linear(encoder.config.hidden_size, num_labels)
    ...
    def forward(
        ...
        """
        # DONE (~6-10 lines, depending on formatting etc): implement forward!
        ...
        output = self.encoder(input_ids=tokens,
